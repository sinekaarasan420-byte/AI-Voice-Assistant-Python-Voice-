...
import webbrowser
import subprocess
from gtts import gTTS      # 🌟 கூகுள் வாய்ஸ் மேஜிக்!
import pygame              # 🌟 ஆடியோவை லேக் இல்லாம பிளே செய்ய!

# pygame மிக்ஸரை ஆன் செய்கிறோம்
pygame.mixer.init()

def speak(text):
    print(f"Jarvis: {text}")
    try:
        # பழைய ஆடியோ பிளே ஆகிட்டு இருந்தா ஸ்டாப் செய்கிறோம்
        if pygame.mixer.music.get_busy(): 
            pygame.mixer.music.stop()
        pygame.mixer.music.unload()
        
        # கூகுள் வாய்ஸ் மூலம் தமிழ் ஆடியோ ஃபைல் உருவாக்குறோம் சார்
        tts = gTTS(text=text, lang='ta') # 'ta' கொடுத்தா தமிழ் கச்சிதமா பேசும்!
        audio_file = "jarvis_voice.mp3"
        tts.save(audio_file)
        
        # ஆடியோவை லோடு செய்து பிளே செய்கிறோம்
        pygame.mixer.music.load(audio_file)
        pygame.mixer.music.play()
        
        # ஆடியோ பேசி முடிக்கும் வரை கோடை வெயிட் பண்ண வைக்கிறோம்
        while pygame.mixer.music.get_busy(): 
            time.sleep(0.1)
            
        # பிளே ஆகி முடிஞ்சதும் ஃபைலை அன்லோடு செய்கிறோம்
        pygame.mixer.music.unload()
...
def jarvis_backend():
    # 🌟 ரன் ஆன உடனே ஜார்விஸ் கூகுள் வாய்ஸ்ல தமிழ்ல பேசுவான் சார்!
